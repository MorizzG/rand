...
static void isaac(ctx) randctx* ctx;
{
...
static void randinit(ctx, flag) randctx* ctx;
word flag;
{
    word i;
    ub4 a, b, c, d, e, f, g, h;
...
static unsigned long ISAAC_Bits(void* junk1, void* junk2) { return rand(&ctx); }

/*-----------------------------------------------------------------------*/

static double ISAAC_U01(void* vpar, void* vsta) { return ISAAC_Bits(vpar, vsta) * unif01_INV32; }
...
unif01_Gen* ucrypto_CreateISAAC(int flag, unsigned int A[]) {
    ...
    util_Assert(co1 == 0, "ucrypto_CreateISAAC:   only 1 generator at a time can be in use");
    co1++;
    ...
    if (A != NULL) addstr_ArrayUint(name, ",   A = ", RANDSIZ, A);
    leng = strlen(name);
